<template>
  <GridLayout id="trans_parent" columns="*" rows="auto, *">
    <SegmentedBar class="seg-bar" v-model="selectedTabItem"  row="0" col="0">
      <SegmentedBarItem title="ĐƠN HÀNG" />
      <SegmentedBarItem title="KHÁCH HÀNG"/>
    </SegmentedBar>
    <GridLayout rows="10, 60, *" columns="50, *, 50" row="1" col="0">
      ...
      <Label :text="selectedDate" class="text-center" row="1" col="1"/>
      ...
      <ListView row="2" col="0" colSpan="3" rowSpan="2" for="item in remoteOrders" @itemTap="onSelectedTransaction">
          <v-template>
            <GridLayout flexDirection="row" rows="*,*,*,*" columns="10,100,*" class="ls-item-check-in">
              ...
              <Label :text="item.abiz_ordercode" class="item-header text-color-primary-dark" textWrap="true" row="0" col="2"/>
        ...
        <!-- <Button class="btn btn-fill-bg" text="TẠO ĐƠN HÀNG" row="3" col="1" @tap="createTransaction()"/> -->
    ...
    <GridLayout rows="10, 60, *" columns="50, *, 50" row="1" col="0" v-if="selectedTabItem == 1" background="white">
